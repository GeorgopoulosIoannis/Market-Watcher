...
import { SymbolIntraday } from './symbol-intraday';
...
export class DashboardService {
	...
	convertIntraday(data: SymbolIntraday): Series[] {
		const arr = Object.entries(data.intraday);
		arr.reverse();

		const dataItems: DataItem[] = arr.map(d => ({
			name: d[0].slice(10),
			value: Number(d[1].open)
		}));
		return [{ name: data.symbol, series: dataItems }];
	}
}
